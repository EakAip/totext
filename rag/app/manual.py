...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    if re.search(r"\.pdf$", filename, re.IGNORECASE):
        pdf_parser = Pdf() if kwargs.get(
            ...
            # "layout_recognize", True) else Pdf()
...
if __name__ == "__main__":
    ...
    # 获取命令行中的文件路径
    file_path = sys.argv[1] if len(sys.argv) > 1 else '/path/to/your/document.pdf'
    
    # 调用 chunk 函数处理 PDF
    results = chunk(file_path, from_page=0, to_page=10, lang="Chinese", callback=dummy)
